...
def plot_image_tuple(bundle):
    ...
    fig = plt.figure(f_num + 1, figsize=(im_size, im_size), dpi=dpi, facecolor='w')
    ax = fig.add_subplot(1, 1, 1)
    ax.set_aspect(ratio)

    cmap, vmin, vmax = get_cmap(cmap_name, sat_name, b_num)
    im = ax.pcolormesh(lons, lats, BT, cmap=cmap, vmin=vmin, vmax=vmax, shading='nearest')

    if show_center and (not no_crop or use_track):
        theta = np.linspace(0, 2 * np.pi, 150)
        if Eye_area_r > 10:
            plt.scatter(lon_TC, lat_TC, s=cm_size / 2500, facecolors=(1, 0, 1, 0.5), edgecolors='none')
        if do_ADT:
            lon_inner = R_inner * np.cos(theta) * ratio + lon_TC
            lat_inner = R_inner * np.sin(theta) + lat_TC
            # parametric circle
            plt.plot(lon_inner, lat_inner, '-.', color=(1, 0, 0, 0.5))
            lon_outer = R_out * np.cos(theta) * ratio + lon_TC
            lat_outer = R_out * np.sin(theta) + lat_TC
            # parametric circle
            plt.plot(lon_outer, lat_outer, '-.', color=(1, 0, 0, 0.5))
        else:
            plt.scatter(lon_TC, lat_TC, s=cm_size, facecolors='none', edgecolors=(1, 0, 0, 0.5))

        lon_edge = R_edge * np.cos(theta) * ratio + lon_TC
        lat_edge = R_edge * np.sin(theta) + lat_TC
        plt.plot(lon_edge, lat_edge, '-.', color=(0, 0.5, 0, 0.5))


    ax.grid()

    if use_track and plot_track:
        ax.plot(xpTrack, ypTrack, '-.', color=(0, 0, 0, 0.5))

    image_type_text = '_'.join([crop_status, cmap_name])

    title_text = ''.join([top_text, '\n', image_type_text, '\nTime valid:', image_datetime_text])
    ...
    if show_RMW:
        ...
        rmw_text = f'R: {np.round(Eye_area_r,acc)} km, RMW: {np.round(r_to_RMW(Eye_area_r, new_RMW), acc)} km'
        plt.title(''.join([rmw_text, '\n', pos_text, '\n', temp_text]), loc='right', fontsize=10)
    else:
        plt.title(''.join([pos_text, '\n', temp_text]), loc='right', fontsize=10)



    if save_image:
        image_name = ''.join([top_text, '_', image_type_text, '_', image_datetime_text])
        target_dir = os.path.join(get_folder(type='output_image'), crop_status, TC, f'Band_{b_num}')
        print(target_dir)
        os.makedirs(target_dir, exist_ok=True)
